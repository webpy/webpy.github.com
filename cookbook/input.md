...
What if you pass several of the same variable names (like in a <select mulitple="multiple"> scenario)?  You need to let web.input know to expect multiple inputs, or it will clobber all but one.  Pass the default value of a list to web.input and it will work correctly.  For example, going to http://example.com?id=10&id=20:
