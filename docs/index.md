...
<a name="nomethod"></a>`nomethod(cls)`
   : Returns a `405 Method Not Allowed` error for `cls`.

<a name="autodelegate"></a>`autodelegate(prefix='')`
...
<a name="webpyfunc"></a>`webpyfunc(inp, fvars, autoreload=False)`
   : If `inp` is a url mapping, returns a function that calls handle.

<a name="run"></a>`run(inp, fvars, *middleware)`
...
<a name="runscgi"></a>`runscgi(func, addr=('localhost', 4000))`
   : Runs a WSGI function as an SCGI server.

<a name="runwsgi"></a>`runwsgi(func)`
...
<a name="expires"></a>`expires(delta)`
   : Outputs an `Expires` header for `delta` from now. 
     `delta` is a `timedelta` object or a number of seconds.

<a name="lastmodified"></a>`lastmodified(date_obj)`
...
<a name="found"></a>`found(url)`
   : A `302 Found` redirect.

<a name="seeother"></a>`seeother(url)`
   : A `303 See Other` redirect.

<a name="tempredirect"></a>`tempredirect(url)`
   : A `307 Temporary Redirect` redirect.

<a name="write"></a>`write(cgi_response)`
   : Converts a standard CGI-style string response into `header` and 
     `output` calls.

<a name="changequery"></a>`changequery(**kw)`
   : Imagine you're at `/foo?a=1&b=2`. Then `changequery(a=3)` will return
     `/foo?a=3&b=2` -- the same URL but with the arguments you requested
     changed.

<a name="background"></a>`background(func)`
...
<a name="render"></a>`render(template, terms=None, asTemplate=False, base=None, isString=False):`
   ...
   : A Storage object is like a dictionary except `obj.foo` can be used
     in addition to `obj['foo']`.
         ...
         Traceback (most recent call last):
             ...
         AttributeError: 'a'
<a name="storify"></a>`storify(mapping, *requireds, **defaults)`
...
<a name="rstrips"></a>`rstrips(text, remove)`
   : removes the string `remove` from the right of `text`
         ...
         >>> rstrips("foobar", "bar")
         'foo'
<a name="lstrips"></a>`lstrips(text, remove)`
   : removes the string `remove` from the left of `text`
     
         >>> lstrips("foobar", "foo")
         'bar'
<a name="strips"></a>`strips(text, remove)`
   : removes the string `remove` from the both sides of `text` 
         >>> strips("foobarfoo", "foo")
         'bar'
<a name="class TimeoutError"></a>`class TimeoutError(Exception): pass def timelimit(timeout):`
...
<a name="re_subm"></a>`re_subm(pat, repl, string)`
...
<a name="dictreverse"></a>`dictreverse(mapping)`
   : >>> dictreverse({1: 2, 3: 4})
         {2: 1, 4: 3}

<a name="dictfind"></a>`dictfind(dictionary, element)`
...
<a name="dictfindall"></a>`dictfindall(dictionary, element)`
...
<a name="dictincr"></a>`dictincr(dictionary, element)`
...
<a name="dictadd"></a>`dictadd(dict_a, dict_b)`
...
<a name="listget"></a>`listget(lst, ind, default=None)`
...
<a name="datestr"></a>`datestr(then, now=None)`
...
<a name="numify"></a>`numify(string)`
   : Removes all non-digit characters from `string`.
         ...
         >>> numify('800-555-1212')
         '8005551212'
<a name="denumify"></a>`denumify(string, pattern)`
...
<a name="class Profile"></a>`class Profile`
   : Profiles `func` and returns a tuple containing its output
     and a string with human-readable profiling information.
         ...
         >>> inf[:10].strip()
         'took 0.0'
<a name="tryall"></a>`tryall(context, prefix=None)`
...
<a name="autoassign"></a>`autoassign(self, locals)`
...
<a name="safemarkdown"></a>`safemarkdown(text)`
...
<a name="internalerror"></a>`internalerror()`
   : Returns a `500 Internal Server` error.

<a name="header"></a>`header(hdr, value, unique=False)`
...
<a name="setcookie"></a>`setcookie(name, value, expires="", domain=None)`
   : Sets a cookie.

<a name="cookies"></a>`cookies(*requireds, **defaults)`
   : Returns a `storage` object with all the cookies in it.
     See `storify` for how `requireds` and `defaults` work.

<a name="debug"></a>`debug(*args)`
...
<a name="class Textarea"></a>`class Textarea(Input)`
<a name="class Dropdown"></a>`class Dropdown(Input)`
<a name="class Radio"></a>`class Radio(Input)`
...
<a name="aparam"></a>`aparam()`
   : Returns the appropriate string to be used to interpolate
     ...
     if there isn't one.
         ...
         >>> aparam()
         '%s'
<a name="reparam"></a>`reparam(string_, dictionary)`
   : Takes a string and a dictionary and interpolates the string
     using values from the dictionary. Returns an `SQLQuery` for the result.
     
         >>> reparam("s = $s", dict(s=True))
         <sql: "s = 't'">
<a name="sqlify"></a>`sqlify(obj)`
...
<a name="sqlquote"></a>`sqlquote(a)`
   : Ensures `a` is quoted properly for use in a SQL query.
     
         >>> 'WHERE x = ' + sqlquote(True) + ' AND y = ' + sqlquote(3)
         <sql: "WHERE x = 't' AND y = 3">
<a name="class UnknownDB"></a>`class UnknownDB(Exception)`
   : raised for unsupported dbms

<a name="connect"></a>`connect(dbn, **keywords)`
...
<a name="query"></a>`query(sql_query, vars=None, processed=False, _test=False)`
...
<a name="update"></a>`update(tables, where, vars=None, _test=False, **values)`
...
<a name="validipport"></a>`validipport(port)`
   : returns True if `port` is a valid IPv4 port

<a name="validip"></a>`validip(ip, defaultaddr="0.0.0.0", defaultport=8080)`
   : returns `(ip_address, port)` from string `ip_addr_port`

<a name="validaddr"></a>`validaddr(string_)`
...
<a name="urlquote"></a>`urlquote(val)`
...
<a name="parsehttpdate"></a>`parsehttpdate(string_)`
   : Parses an HTTP date into a datetime object.
 
         >>> parsehttpdate('Thu, 01 Jan 1970 01:01:01 GMT')
         datetime.datetime(1970, 1, 1, 1, 1, 1)

<a name="htmlquote"></a>`htmlquote(text)`
   : Encodes `text` for raw use in HTML.
     
         >>> htmlquote("<'&\\">")
         '<'&">'
<a name="websafe"></a>`websafe(val)`
